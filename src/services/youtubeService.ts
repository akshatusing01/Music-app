...
export const initYouTubeAuth = (_onAuthSuccess?: unknown, onAuthFailure?: () => void) => {
  if (!cachedAccessToken) onAuthFailure?.();
  return () => undefined;
};

export const signInWithYouTubeGoogle = async (): Promise<null> => {
  throw new Error('Google YouTube account import is temporarily unavailable. Use the YouTube playlist URL importer.');
};
...
const youtubeFetch = async (url: string, accessToken: string) => {
  ...
  if (!res.ok) { const body = await res.json().catch(() => ({})); throw new Error(body.error?.message || `YouTube API Error (${res.status})`); }
  return res.json();
};
...
export const fetchPlaylistTracks = async (accessToken: string, playlistId: string): Promise<YouTubePlaylistItem[]> => {
  ...
  return (data.items || []).filter((item: any) => item.snippet?.title && !['Private video', 'Deleted video'].includes(item.snippet.title)).map((item: any) => {
...
export const playlistItemToSong = (item: YouTubePlaylistItem): Song => ({ id: item.videoId, title: item.title, artist: item.artist, artwork: item.thumbnailUrl, youtubeVideoId: item.videoId });
export const playlistMetaToPlaylist = (meta: YouTubePlaylistMeta, tracks: YouTubePlaylistItem[]): Playlist => ({ id: meta.id, name: meta.title, description: meta.description, coverUrl: meta.thumbnailUrl, songIds: tracks.map((track) => track.videoId), isPublic: meta.privacyStatus === 'public' });
