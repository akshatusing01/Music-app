...
export const AudioPlayerBar: React.FC<AudioPlayerBarProps> = (props) => {
  const {
    currentSong, isPlaying, onTogglePlay, onNext, onPrev, playbackPosition, onSeek,
    quality, onChangeQuality, onOpenLyrics, onOpenEqualizer, onOpenShare,
    isDownloaded, onToggleDownload, isShuffle, onToggleShuffle, repeatMode, onCycleRepeat,
  } = props;
  ...
  const seekTo = (target: number) => {
    ...
    if (isYouTube) youtubePlayer.seek(next); else audioEngine.seek(next);
    onSeek(next);
  };
  ...
  const handleTogglePlay = () => {
    ...
    if (actualPlaying) youtubePlayer.pause(); else youtubePlayer.play();
    onTogglePlay();
  };
  ...
  const Progress = ({ expanded = false }: { expanded?: boolean }) => (
    <div
      ...
      tabIndex={0}
      onPointerDown={!expanded ? handleProgressPointerDown : undefined}
      onKeyDown={!expanded ? (event) => { if (event.key === 'ArrowLeft') skip(-5); if (event.key === 'ArrowRight') skip(5); } : undefined}
      className={`group relative w-full cursor-pointer touch-none bg-white/10 ${expanded ? 'h-1.5 rounded-full' : 'h-1'}`}
    >
  ...
  return (
    <>
      {/* The YouTube iframe is only the playback provider. Cineosync owns the player UI. */}
      <div
        ref={youtubeHostRef}
        aria-hidden={!videoVisible}
        className={videoVisible
          ? 'pointer-events-auto fixed left-[5vw] top-[max(78px,12vh)] z-[111] aspect-video w-[90vw] max-w-[900px] overflow-hidden rounded-2xl border border-white/15 bg-black shadow-2xl md:left-1/2 md:w-[min(82vw,900px)] md:-translate-x-1/2'
          ...
          <div className="relative z-[112] mx-auto flex w-full max-w-2xl flex-1 flex-col justify-end px-5 pb-7 pt-2 sm:px-10">
            ...
            <div className="mb-4 flex items-end gap-3">
              <div className="min-w-0 flex-1">
                <h2 className="truncate font-serif text-xl font-semibold tracking-tight sm:text-3xl">{currentSong.title}</h2>
                <p className="mt-1 truncate text-sm text-zinc-400">{currentSong.artist}</p>
              </div>
      ...
      {/* Mobile: sits ABOVE the fixed bottom navigation instead of underneath it. Desktop: remains flush to the viewport bottom. */}
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 sm:grid-cols-[minmax(220px,1fr)_auto_minmax(220px,1fr)] sm:gap-5">
            ...
            <div className="hidden items-center justify-end gap-1 sm:flex">
              ...
              <div className="relative">
                ...
                {qualityMenuOpen && <div className="absolute bottom-10 right-0 z-[120] w-32 rounded-xl border border-white/10 bg-[#121214] p-1 shadow-2xl">
                  {(['low-128k', 'medium-256k', 'high-320k'] as AudioQuality[]).map((q) => <button key={q} type="button" onClick={() => { onChangeQuality(q); setQualityMenuOpen(false); }} className={`block w-full rounded-lg px-3 py-2 text-left text-xs hover:bg-white/10 ${quality === q ? 'text-white' : 'text-zinc-400'}`}>{q.replace('-', ' ')}</button>)}
                </div>}
              </div>
            </div>
          </div>
