...
export const AudioPlayerBar: React.FC<AudioPlayerBarProps> = ({
  ...
  useEffect(() => {
    ...
    youtubePlayer.mount(youtubeHostRef.current).catch((error) => {
  ...
  useEffect(() => {
    ...
    youtubePlayer.load(currentSong.youtubeVideoId, playbackPosition, isPlaying, 1).catch((error) => {
      console.warn('Unable to load YouTube track', error);
    });
  ...
  const seekFromClientX = (clientX: number) => {
    ...
    if (isYouTube) youtubePlayer.seek(target);
    onSeek(target);
  };
  ...
  const skip = (amount: number) => {
    const target = Math.max(0, Math.min(duration, position + amount));
    if (isYouTube) youtubePlayer.seek(target);
    onSeek(target);
  };
      ...
      <div className="mx-auto w-full max-w-[1600px] px-3 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 sm:px-5 sm:py-2.5">
        <div className="grid grid-cols-1 gap-2 lg:grid-cols-[minmax(280px,1fr)_auto_minmax(280px,1fr)] lg:items-center lg:gap-6">
          <div className="flex min-w-0 items-center gap-2.5 lg:max-w-[560px]">
            {isYouTube && (
              <div className="relative h-[68px] w-[120px] shrink-0 overflow-hidden rounded-lg border border-white/10 bg-black sm:h-[78px] sm:w-[138px]" aria-label="YouTube player">
                <div ref={youtubeHostRef} className="h-full w-full" />
              </div>
            )}
            <button type="button" onClick={onOpenLyrics} className="relative h-11 w-11 shrink-0 overflow-hidden rounded-md border border-white/10 bg-white/5 sm:h-12 sm:w-12" title="Open now playing">
              <img src={currentSong.coverArt} alt="" referrerPolicy="no-referrer" className="h-full w-full object-cover" />
              <span className="absolute inset-0 hidden place-items-center bg-black/50 hover:grid"><ChevronUp size={18} /></span>
            </button>
            <button type="button" onClick={onOpenLyrics} className="min-w-0 flex-1 text-left">
              ...
              <div className="mt-0.5 text-[9px] font-semibold uppercase tracking-widest text-zinc-600">{isYouTube ? 'YouTube · official player' : 'Authorized stream'}</div>
            </button>
            <div className="hidden shrink-0 items-center gap-0.5 sm:flex">
              <button type="button" onClick={toggleLike} className={`${iconButton} ${isLiked ? 'text-rose-400' : ''}`} aria-label="Like" title="Like"><Heart size={17} fill={isLiked ? 'currentColor' : 'none'} /></button>
              <button type="button" onClick={toggleDislike} className={`${iconButton} ${isDisliked ? 'text-white' : ''}`} aria-label="Dislike" title="Dislike"><ThumbsDown size={16} fill={isDisliked ? 'currentColor' : 'none'} /></button>
            </div>
          </div>
          ...
          <div className="flex min-w-0 items-center justify-center gap-0.5 sm:gap-1.5">
            ...
            <button type="button" onClick={handleTogglePlay} className="mx-1 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-black shadow-lg transition-transform hover:scale-105 active:scale-95 sm:h-11 sm:w-11" aria-label={isPlaying ? 'Pause' : 'Play'}>
          ...
          <div className="flex min-w-0 items-center justify-center gap-0.5 sm:justify-end sm:gap-1">
            ...
            <div className="hidden items-center gap-1 pl-1 sm:flex">
              <button type="button" onClick={() => setIsMuted((value) => !value)} className={iconButton} aria-label={isMuted ? 'Unmute' : 'Mute'}>
                {isMuted || volume === 0 ? <VolumeX size={17} /> : volume < 0.5 ? <Volume1 size={17} /> : <Volume2 size={17} />}
              </button>
              <input aria-label="Volume" type="range" min="0" max="1" step="0.01" value={isMuted ? 0 : volume} onChange={(event) => { setVolume(Number(event.target.value)); setIsMuted(false); }} className="h-1 w-16 cursor-pointer accent-rose-500 lg:w-20" />
            </div>
            <div className="relative hidden md:block">
              <button type="button" onClick={() => setQualityMenuOpen((open) => !open)} className="rounded-full border border-white/10 bg-white/5 px-2 py-1 text-[9px] font-bold uppercase tracking-wider text-zinc-300 hover:bg-white/10">{quality === 'data-saver-64k' ? '64k' : quality === 'normal-128k' ? '128k' : 'HD'}</button>
              {qualityMenuOpen && <div className="absolute bottom-full right-0 z-[60] mb-2 w-44 rounded-xl border border-white/10 bg-zinc-900 p-1.5 shadow-2xl">
                {(['data-saver-64k', 'normal-128k', 'high-320k'] as AudioQuality[]).map((item) => <button type="button" key={item} onClick={() => { onChangeQuality(item); setQualityMenuOpen(false); }} className={`w-full rounded-lg px-2.5 py-2 text-left text-xs ${quality === item ? 'bg-rose-500/15 text-rose-300' : 'text-zinc-300 hover:bg-white/5'}`}>{item === 'data-saver-64k' ? 'Data Saver · 64k' : item === 'normal-128k' ? 'Standard · 128k' : 'High · 320k'}</button>)}
              </div>}
            </div>
          </div>
        </div>
        ...
        <div className="mt-1 flex items-center justify-between px-1 text-[9px] font-mono text-zinc-500 sm:hidden">
          <span>{formatTime(position)}</span><span>{isYouTube ? 'YOUTUBE' : 'STREAM'}</span><span>{formatTime(duration)}</span>
        </div>
      </div>
    </div>
  );
