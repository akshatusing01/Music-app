...
export function App() {
  const [userProfile, setUserProfile] = useState<UserProfile>(() => persistenceService.getProfile() ?? { id: 'local-' + Math.random().toString(36).slice(2, 9), name: 'Listener', avatar: '', statusMessage: 'Listening on SyncBeat', presenceMode: 'available-to-join', language: 'en', theme: 'neon-obsidian', quality: 'high-320k', isWifiOnlyDownloads: true, favoriteGenres: [], stats: { minutesListened: 0, sessionsJoined: 0, focusHours: 0, streakDays: 0 } });
  ...
  useEffect(() => { persistenceService.setProfile({ ...userProfile, language, theme }); cloudPersistenceService.saveProfile({ ...userProfile, language, theme }).catch(() => undefined); }, [userProfile, language, theme]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('syncbeat:cloud-restored', handler);
    return () => window.removeEventListener('syncbeat:cloud-restored', handler);
  }, []);
  ...
  useEffect(() => { if (!timerState.isRunning || timerState.remaining <= 0) return; const i = window.setInterval(() => setTimerState((p) => p.remaining <= 1 ? { ...p, remaining: 0, isRunning: false, completedSessions: p.completedSessions + 1 } : { ...p, remaining: p.remaining - 1 }), 1000); return () => clearInterval(i); }, [timerState.isRunning, timerState.remaining]);
  ...
  const handleTogglePlay = useCallback(() => { if (!currentSong) return; const position = audioEngine.getCurrentPosition(); if (isPlaying) { audioEngine.pause(); setIsPlaying(false); if (activeRoom) wsClient.broadcastPlayback('PLAY_PAUSE', { isPlaying: false, position, senderName: userProfile.name }); } else { audioEngine.resume(); setIsPlaying(true); if (activeRoom) wsClient.broadcastPlayback('PLAY_PAUSE', { isPlaying: true, position, senderName: userProfile.name }); } }, [currentSong, isPlaying, activeRoom, userProfile.name]);
  const handleNextTrack = useCallback(() => { if (!queue.length) return; const currentIndex = queue.findIndex((s) => s.id === currentSong?.id); const nextIndex = isShuffle ? Math.floor(Math.random() * queue.length) : repeatMode === 'one' ? Math.max(currentIndex, 0) : (currentIndex + 1) % queue.length; if (queue[nextIndex]) handlePlaySong(queue[nextIndex]); }, [queue, currentSong, isShuffle, repeatMode, handlePlaySong]);
  ...
  const handleToggleLike = (songId: string) => { const song = songs.find((s) => s.id === songId) ?? currentSong ?? undefined; setLikedSongIds((prev) => { const next = new Set(prev); const liked = !next.has(songId); liked ? next.add(songId) : next.delete(songId); if (song) cloudPersistenceService.setLiked(song, liked).catch(() => undefined); return next; }); };
  ...
  const handleToggleTimer = () => setTimerState((p) => ({ ...p, isRunning: !p.isRunning }));
  const handleResetTimer = () => setTimerState((p) => ({ ...p, remaining: p.duration, isRunning: false }));
  ...
  </main><RightContextPanel activeRoom={activeRoom} latencyMs={latencyMs} onSelectPlaylist={handleSelectPlaylist} onOpenShare={() => setIsShareOpen(true)} /><AudioPlayerBar {...commonProps} onToggleLike={() => currentSong && handleToggleLike(currentSong.id)} isLiked={!!currentSong && likedSongIds.has(currentSong.id)} onToggleShuffle={() => setIsShuffle((p) => !p)} isShuffle={isShuffle} repeatMode={repeatMode} onCycleRepeat={() => setRepeatMode((p) => p === 'off' ? 'all' : p === 'all' ? 'one' : 'off')} onOpenEqualizer={() => setIsEqualizerOpen(true)} onOpenShare={() => setIsShareOpen(true)} /><AuthController />{isEqualizerOpen && <EqualizerModal onClose={() => setIsEqualizerOpen(false)} />}{isShareOpen && <ShareModal onClose={() => setIsShareOpen(false)} song={currentSong} />}{isAiGeneratorOpen && <AIGeneratorModal onClose={() => setIsAiGeneratorOpen(false)} onAddToQueue={handleAddToQueue} />}</div>;
}
