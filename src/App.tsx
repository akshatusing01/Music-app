...
export function App() {
  ...
  const handleJoinRoom = useCallback((roomId: string) => { let id = roomId.trim(); try { const parsed = new URL(id, window.location.origin); id = parsed.searchParams.get('room') || id; } catch {} wsClient.joinRoom(id, userProfile.name, userProfile.avatar); setCurrentTab('sessions'); }, [userProfile.name, userProfile.avatar]);
