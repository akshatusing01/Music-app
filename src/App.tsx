...
export function App() {
  ...
  useEffect(() => {
    const onCloudRestored = (event: Event) => { const data = (event as CustomEvent).detail; if (!data) return; if (Array.isArray(data.likedIds)) setLikedSongIds(new Set(data.likedIds)); if (Array.isArray(data.playlists)) setPlaylists(data.playlists); if (data.profile) setUserProfile((prev) => ({ ...prev, name: data.profile.display_name ?? prev.name, avatar: data.profile.avatar_url ?? prev.avatar, statusMessage: data.profile.status_message ?? prev.statusMessage, presenceMode: data.profile.presence_mode ?? prev.presenceMode, language: data.profile.language ?? prev.language, theme: data.profile.theme ?? prev.theme, quality: data.profile.audio_quality ?? prev.quality, favoriteGenres: data.profile.favorite_genres ?? prev.favoriteGenres })); };
    window.addEventListener('syncbeat:cloud-restored', onCloudRestored); return () => window.removeEventListener('syncbeat:cloud-restored', onCloudRestored);
  }, []);
  ...
  useEffect(() => { const unsub = wsClient.addListener((event) => { if (event.type === 'ROOM_SYNC_STATE') setActiveRoom(event.payload as RoomState); if (event.type === 'ROOM_NOT_FOUND') setRoomError(`No public room named “${event.payload?.roomName || event.payload?.roomId || 'that'}” exists.`); if (event.type === 'SESSION_RECONNECTING') setRoomError('Reconnecting to room…'); if (event.type === 'SESSION_CONNECTED') setRoomError(''); }); return () => unsub(); }, []);
  ...
  const handlePlaySong = useCallback((song: Song) => {
    ...
    setCurrentSong(song); setPlaybackPosition(0); setIsPlaying(true); setQueue((prev) => prev.some((s) => s.id === song.id) ? prev : [song, ...prev]);
    if (song.youtubeVideoId) void youtubePlayer.load(song.youtubeVideoId, 0, true, 1); else void audioEngine.playSong(song, 0);
    const source = activeRoom ? 'session' : 'app'; persistenceService.addHistory({ song, playedAt: Date.now(), source }); cloudPersistenceService.recordHistory(song, source).catch(() => undefined); window.dispatchEvent(new Event('syncbeat:history-updated'));
  ...
  const handleTogglePlay = useCallback(() => {
    ...
    if (activeRoom && !wsClient.isHost()) { void wsClient.resyncFromHost(); return; }
    const position = currentSong.youtubeVideoId ? youtubePlayer.getCurrentTime() : audioEngine.getCurrentPosition();
    if (currentSong.youtubeVideoId) { if (isPlaying) { youtubePlayer.pause(); setIsPlaying(false); } else { youtubePlayer.play(); setIsPlaying(true); } }
    else if (isPlaying) { audioEngine.pause(); setIsPlaying(false); } else { void audioEngine.resume(); setIsPlaying(true); }
    if (activeRoom) wsClient.broadcastPlayback('PLAY_PAUSE', { isPlaying: !isPlaying, position, senderName: userProfile.name });
  }, [currentSong, isPlaying, activeRoom, userProfile.name]);

  const handleNextTrack = useCallback(() => { if (activeRoom && !wsClient.isHost()) { void wsClient.resyncFromHost(); return; } if (!queue.length) return; const currentIndex = queue.findIndex((s) => s.id === currentSong?.id); const nextIndex = repeatMode === 'one' ? Math.max(currentIndex, 0) : isShuffle ? Math.floor(Math.random() * queue.length) : (currentIndex + 1) % queue.length; if (queue[nextIndex]) handlePlaySong(queue[nextIndex]); }, [queue, currentSong, isShuffle, repeatMode, handlePlaySong, activeRoom]);
  ...
  const handleSeek = useCallback((seconds: number) => { if (activeRoom && !wsClient.isHost()) { void wsClient.resyncFromHost(); return; } if (currentSong?.youtubeVideoId) { youtubePlayer.seek(seconds); } else { audioEngine.seek(seconds); } setPlaybackPosition(seconds); if (activeRoom) wsClient.broadcastPlayback('SEEK', { position: seconds, isPlaying, senderName: userProfile.name }); }, [activeRoom, isPlaying, userProfile.name, currentSong]);
  ...
  const handleJoinRoom = useCallback((roomOrName: string) => { const value = roomOrName.trim(); if (!value) return; const isLink = value.includes('://') || value.includes('?room='); const isRoomId = /^room-[a-z0-9]+$/i.test(value); setRoomError(''); if (isLink || isRoomId) wsClient.joinRoom(value, userProfile.name, userProfile.avatar); else void wsClient.joinRoomByName(value, userProfile.name, userProfile.avatar); setCurrentTab('sessions'); }, [userProfile.name, userProfile.avatar]);
  ...
  return <div className="min-h-screen bg-zinc-950 text-white">
    {roomError && <button onClick={() => setRoomError('')} className="fixed left-1/2 top-16 z-[80] -translate-x-1/2 max-w-[90vw] rounded-full border border-amber-500/30 bg-amber-950/90 px-4 py-2 text-[11px] font-semibold text-amber-100 shadow-2xl backdrop-blur">{roomError}</button>}
    ...
    <main className="min-h-screen px-4 pb-72 pt-20 sm:pb-32 lg:pl-64 lg:pr-80">
      ...
      {currentTab === 'sessions' && <div className="space-y-5"><SessionsView room={activeRoom} currentSong={currentSong} isPlaying={isPlaying} onTogglePlay={handleTogglePlay} onNextTrack={handleNextTrack} onSelectSong={onSelectSessionSong} availableSongs={songs} currentUser={{ id: userProfile.id, name: userProfile.name, avatar: userProfile.avatar }} onCreateRoom={handleCreateRoom} onJoinRoom={handleJoinRoom} onLeaveRoom={handleLeaveRoom} onOpenLyrics={onOpenLyrics} language={language} latencyMs={latencyMs} />{activeRoom && <RoomInvitePanel room={activeRoom} />}</div>}
      ...
      {currentTab === 'importer' && <PlaylistImporterView availableSongs={songs} playlists={playlists} onImportPlaylist={handleImportPlaylist} onPlayPlaylist={onPlayPlaylist} language={language} onPlaySong={handlePlaySong} />}
    </main>
