export type SupportedLanguage =
  | 'en' | 'hi' | 'ta' | 'te' | 'pa' | 'bn' | 'mr' | 'kn' | 'ml' | 'gu' | 'ur';

export type AppTheme = 'neon-obsidian' | 'bollywood-ruby' | 'focus-emerald' | 'sapphire-gym' | 'chill-violet' | 'glass-light';
export type ExperienceMode = 'standard' | 'love' | 'focus' | 'gym' | 'friends' | 'chill' | 'bollywood' | 'regional' | 'custom';
export type AudioQuality = 'data-saver-64k' | 'normal-128k' | 'high-320k';
export type SyncState = 'SYNCED' | 'SYNCING' | 'RECONNECTING' | 'OFFLINE';
export type MoodCategory = 'all' | 'romance' | 'gym' | 'study' | 'party' | 'devotional' | 'indie' | 'chill' | 'regional' | 'bollywood';

export interface LyricLine { time: number; text: string; transliteration?: string; translation?: string; }
...
export interface Song {
  ...
  sourceProvider?: 'SyncBeat' | 'Spotify' | 'YouTube Music' | 'YouTube' | 'Apple Music' | 'Amazon Music';
}

export interface Playlist { id: string; title: string; description: string; coverArt: string; mood: MoodCategory; songIds: string[]; isAiGenerated?: boolean; themeGradient?: string; isCurated?: boolean; creatorName?: string; platformSource?: 'Spotify' | 'YouTube' | 'Apple' | 'Amazon' | 'SyncBeat' | string; experienceMode?: ExperienceMode; }
export interface RoomParticipant { id: string; name: string; avatar: string; isHost: boolean; joinedAt: number; lastPing: number; deviceType?: 'phone' | 'desktop' | 'tablet' | 'headphones'; currentStatus?: string; }
export interface ChatMessage { id: string; senderId: string; senderName: string; senderAvatar: string; text: string; type: 'text' | 'reaction' | 'system' | 'sound' | 'moment'; timestamp: number; reactionEmoji?: string; soundName?: string; trackPosition?: number; songTitle?: string; }
export interface FloatingReaction { id: string; emoji: string; senderName: string; x: number; createdAt: number; }
export interface FocusTimerState { active: boolean; timerType: 'pomodoro' | 'stopwatch' | 'idle'; duration: number; remaining: number; isRunning: boolean; startedAt: number | null; mode: 'work' | 'shortBreak' | 'longBreak'; completedSessions: number; presetName?: '25/5' | '50/10' | '90min' | 'custom'; }
