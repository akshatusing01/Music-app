...
type Room = {
  ...
  version: number; queue: string[]; participants: Map<string, Participant>; clients: Map<WebSocket, string>; chatMessages: any[]; focusMode: FocusMode;
};

const rooms = new Map<string, Room>();
function createRoom(roomId: string, participant: Participant, options: any = {}): Room { return { roomId, roomName: String(options.roomName || `Cineosync Room ${roomId}`).slice(0, 80), moodTheme: String(options.moodTheme || 'vibe').slice(0, 30), hostId: participant.id, isPublic: options.isPublic !== false, currentSongId: typeof options.initialSongId === 'string' ? options.initialSongId : null, currentSong: options.initialSong && typeof options.initialSong === 'object' ? options.initialSong : null, isPlaying: false, playbackPosition: 0, playbackRate: 1, lastStateUpdate: Date.now(), version: 0, queue: options.initialSongId ? [options.initialSongId] : [], participants: new Map(), clients: new Map(), chatMessages: [], focusMode: { active: false, timerType: 'idle', duration: 1500, remaining: 1500, isRunning: false, startedAt: null } }; }
...
function bump(room: Room) { room.version += 1; room.lastStateUpdate = Date.now(); }
function fallbackYouTubeSong(songId: string) { if (!songId.startsWith('yt-search-')) return null; const videoId = songId.slice('yt-search-'.length); return { id: songId, title: 'Shared track', artist: 'Shared source', album: 'Music', duration: 0, coverArt: `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`, language: 'en', languageLabel: 'Music', mood: 'chill', tags: ['session'], youtubeVideoId: videoId, lyrics: [], sourceProvider: 'YouTube' }; }

const server = http.createServer((_req, res) => { res.writeHead(200, { 'content-type': 'application/json', 'cache-control': 'no-store' }); res.end(JSON.stringify({ status: 'ok', service: 'cineosync-session-websocket' })); });
  ...
  ws.on('message', (raw) => {
    try {
      ...
      if (type === 'JOIN_ROOM') {
        ...
        const systemMessage = { id: `msg-${Date.now()}`, senderId: 'system', senderName: 'Cineosync', senderAvatar: '', text: `${participant.name} joined the room.`, type: 'system', timestamp: Date.now() };
        room.chatMessages.push(systemMessage); bump(room); broadcast(room, 'PARTICIPANT_JOINED', { participant, participants: [...room.participants.values()], systemMessage, version: room.version }, ws); return;
      }
      if (!room || !participantId) return;
      if (type === 'PLAYBACK_ACTION') {
        if (!isHost(room, participantId)) { send(ws, 'HOST_ONLY_PLAYBACK', serializeRoom(room)); return; }
        const action = payload.action;
        if (action === 'PLAY_PAUSE') { room.playbackPosition = Number.isFinite(Number(payload.position)) ? Math.max(0, Number(payload.position)) : currentPosition(room); room.isPlaying = Boolean(payload.isPlaying); }
        ...
        bump(room); broadcastAll(room, 'PLAYBACK_SYNC', { version: room.version, serverTimestamp: room.lastStateUpdate, currentSongId: room.currentSongId, songId: room.currentSongId, song: room.currentSong, isPlaying: room.isPlaying, playbackPosition: room.playbackPosition, position: room.playbackPosition, playbackRate: room.playbackRate, lastStateUpdate: room.lastStateUpdate, action, actionBy: participantId }); return;
      }
      if (type === 'QUEUE_UPDATE') { if (!isHost(room, participantId)) return; const rawQueue: unknown[] = Array.isArray(payload.queue) ? payload.queue : []; room.queue = [...new Set(rawQueue.filter((id: unknown): id is string => typeof id === 'string' && id.length > 0).slice(0, 100))]; bump(room); broadcastAll(room, 'QUEUE_SYNC', { queue: room.queue, version: room.version, serverTimestamp: room.lastStateUpdate, updatedBy: participantId }); return; }
      if (type === 'TRANSFER_HOST') { if (!isHost(room, participantId)) return; const targetId = String(payload.participantId || ''); if (!room.participants.has(targetId) || targetId === room.hostId) return; room.hostId = targetId; room.participants.forEach((p) => { p.isHost = p.id === room!.hostId; }); const target = room.participants.get(targetId); const systemMessage = { id: `msg-${Date.now()}`, senderId: 'system', senderName: 'Cineosync', senderAvatar: '', text: `${target?.name || 'A listener'} is now the host.`, type: 'system', timestamp: Date.now() }; room.chatMessages.push(systemMessage); bump(room); broadcastAll(room, 'HOST_CHANGED', { hostId: room.hostId, participants: [...room.participants.values()], systemMessage, version: room.version }); return; }
