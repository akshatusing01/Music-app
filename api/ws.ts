...
function serializeRoom(room: Room) {
  return {
    ...
    currentSongId: room.currentSongId, isPlaying: room.isPlaying, playbackPosition: currentPosition(room),
    playbackRate: room.playbackRate, lastStateUpdate: room.lastStateUpdate, queue: room.queue,
    participants: [...room.participants.values()], chatMessages: room.chatMessages.slice(-50), focusMode: room.focusMode, isPublic: room.isPublic,
  };
}
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    try {
      ...
      if (type === 'JOIN_ROOM') {
        ...
        broadcast(room, 'PARTICIPANT_JOINED', { participant, participants: [...room.participants.values()], systemMessage }, ws);
        return;
      }
      ...
      if (type === 'PLAYBACK_ACTION') {
        ...
        broadcast(room, 'PLAYBACK_SYNC', { currentSongId: room.currentSongId, songId: room.currentSongId, isPlaying: room.isPlaying, playbackPosition: room.playbackPosition, position: room.playbackPosition, playbackRate: room.playbackRate, lastStateUpdate: room.lastStateUpdate, actionBy: participantId });
        return;
      }

      if (type === 'QUEUE_UPDATE') {
        const queue = Array.isArray(payload.queue) ? payload.queue.filter((id: unknown): id is string => typeof id === 'string' && id.length > 0).slice(0, 100) : [];
        room.queue = [...new Set(queue)];
        broadcast(room, 'QUEUE_SYNC', { queue: room.queue });
        return;
      }
      ...
      if (type === 'SEND_CHAT') {
        ...
        broadcast(room, 'NEW_CHAT_MESSAGE', msg);
        return;
      }

      if (type === 'BURST_REACTION') {
        broadcast(room, 'REACTION_BURST', { emoji: String(payload.emoji || '❤️').slice(0, 8), senderId: participantId, senderName: payload.senderName, x: Math.max(0, Math.min(1, Number(payload.x) || 0.5)), soundEffect: payload.soundEffect });
        return;
      }
      ...
      if (type === 'FOCUS_TIMER_ACTION') {
        ...
        broadcast(room, 'FOCUS_TIMER_SYNC', f);
        return;
      }
  ...
  ws.on('close', () => {
    ...
    if (room.hostId === participantId && room.participants.size > 0) {
      const next = [...room.participants.values()][0]; room.hostId = next.id;
